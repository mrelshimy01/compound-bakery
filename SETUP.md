...
The frontend package is now configured with the supplied Apps Script `/exec` URL.


## If the menu is blank

Run `setupSheets()` manually once from the Apps Script editor and approve authorization.
Then open the `Products` tab and confirm it contains rows under:

`ID | Product | Category | Price | Emoji | Active`

The starter script will create four sample products automatically if the Products tab has no product rows.
